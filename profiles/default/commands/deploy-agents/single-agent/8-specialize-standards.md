...
EXISTING_STANDARDS=$(find geist/standards -name "*.md" -type f 2>/dev/null)
...
echo "   Reference: geist/basepoints/ for module-specific patterns."
```
...
if [ "$PROJECT_NATURE" = "simple" ]; then
    ...
    for optional in "${OPTIONAL_STANDARDS[@]}"; do
        OPTIONAL_FILE="geist/standards/global/$optional"
...
VALIDATION_FILE="geist/standards/global/validation-commands.md"
...
IMPL_VALIDATION_FILE="geist/workflows/validation/validate-implementation.md"
...
if [ -n "$PROJECT_SPECIFIC" ]; then
    ...
    mkdir -p geist/standards/project
    ...
    for pattern_type in $PROJECT_SPECIFIC; do
        ...
        cat > "geist/standards/project/${PATTERN_NAME}.md" << EOF
...
```bash
# Count standards
STANDARDS_COUNT=$(find geist/standards -name "*.md" -type f | wc -l | tr -d ' ')

# Log to reports
mkdir -p geist/output/deploy-agents/reports
cat > geist/output/deploy-agents/reports/standards-specialization.md << EOF
...
## Standards Processed
$(find geist/standards -name "*.md" -type f | while read f; do
    echo "- $(basename "$f" .md): $(dirname "$f" | sed 's|geist/standards/||')"
done)
...
They remain accessible in basepoints: \`geist/basepoints/\`
...
echo "📁 Report saved to: geist/output/deploy-agents/reports/standards-specialization.md"
```
