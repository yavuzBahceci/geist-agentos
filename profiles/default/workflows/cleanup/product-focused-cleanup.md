...
mkdir -p geist/output/product-cleanup
cat > geist/output/product-cleanup/detected-scope.yml << EOF
...
detection_sources:
  tech_stack: geist/product/tech-stack.md
  mission: geist/product/mission.md
  roadmap: geist/product/roadmap.md
EOF
...
```bash
echo ""
echo "🧹 Phase A: Simplifying geist files..."
...
# Process directories
for dir in geist/commands geist/workflows geist/standards geist/agents; do
    if [ -d "$dir" ]; then
...
echo "   5. Add validated patterns to geist files"

# Save search queries for reference
cat > geist/output/product-cleanup/search-queries.md << EOF
...
echo "✅ Search queries saved to geist/output/product-cleanup/search-queries.md"
```
...
# Generate the report
cat > geist/output/product-cleanup/cleanup-report.md << EOF
...
3. Add validated patterns to relevant geist files
4. Re-run this workflow to verify completeness

---

*This report was generated by the product-focused-cleanup workflow.*
*Location: geist/output/product-cleanup/cleanup-report.md*
EOF

echo "✅ Report generated: geist/output/product-cleanup/cleanup-report.md"
```
