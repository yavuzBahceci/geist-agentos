...
if [ -n "$DETECTED_LANGUAGE" ]; then
    ...
    echo "   ✓ Language: $DETECTED_LANGUAGE"
    [ -n "$DETECTED_FRAMEWORK" ] && echo "   ✓ Framework: $DETECTED_FRAMEWORK"
    [ -n "$DETECTED_DATABASE" ] && echo "   ✓ Database: $DETECTED_DATABASE"
fi

# 2. Detect Commands
echo "⚙️  Detecting build/test/lint commands..."
...
if [ -n "$DETECTED_BUILD_CMD" ] || [ -n "$DETECTED_TEST_CMD" ]; then
    DETECTIONS_SUCCESS=$((DETECTIONS_SUCCESS + 1))
    [ -n "$DETECTED_BUILD_CMD" ] && echo "   ✓ Build: $DETECTED_BUILD_CMD"
    [ -n "$DETECTED_TEST_CMD" ] && echo "   ✓ Test: $DETECTED_TEST_CMD"
    [ -n "$DETECTED_LINT_CMD" ] && echo "   ✓ Lint: $DETECTED_LINT_CMD"
fi
...
if [ -n "$DETECTED_PROJECT_TYPE" ]; then
    ...
    [ -n "$DETECTED_MODULES" ] && echo "   ✓ Modules: $DETECTED_MODULES"
fi

# 4. Detect Security Level
echo "🔒 Checking security indicators..."
...
if [ -n "$DETECTED_SECURITY_LEVEL" ]; then
    DETECTIONS_SUCCESS=$((DETECTIONS_SUCCESS + 1))
    echo "   ✓ Security Level: $DETECTED_SECURITY_LEVEL"
fi
```
...
echo ""
echo "📊 Detection confidence: ${DETECTION_CONFIDENCE} ($DETECTIONS_SUCCESS/$DETECTIONS_RUN successful)"
```
...
if [ -z "$DETECTED_LANGUAGE" ]; then
    echo "⚠️  Could not detect language - will ask user"
...
if [ -z "$DETECTED_PROJECT_TYPE" ]; then
    echo "⚠️  Could not detect project type - will ask user"
    DETECTED_PROJECT_TYPE="unknown"
...
if [ -z "$DETECTED_BUILD_CMD" ] && [ -z "$DETECTED_TEST_CMD" ]; then
    echo "⚠️  Could not detect build/test commands - using language defaults"
...
echo ""
echo "✅ Project profile saved to: agent-os/config/project-profile.yml"
```
